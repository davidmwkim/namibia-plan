...
(function (root, factory) {
  ...
  // Resolve a PlacePhoto to a usable <img src>. The Maps JavaScript Places
  // library returns PlacePhoto objects with a `.getUrl({maxWidth})` method and
  // NO `photo_reference` string (that field only exists in the Places Web
  // Service API). getUrl() returns a pre-signed URL that needs no API key. For
  // web-service-shaped fixtures (tests), fall back to any direct `.url`.
  function resolvePhotoUrl(photo, maxWidth) {
    if (!photo) return null;
    if (typeof photo.getUrl === 'function') {
      try { return photo.getUrl({ maxWidth: maxWidth || 800 }); } catch (_) { return null; }
    }
  ...
  function shapePlaceForStop(stop, details) {
    ...
    // Resolve the cover + a few alternates to real image URLs (JS SDK getUrl).
    const photoUrl = resolvePhotoUrl(cover, 800);
    const altPhotoUrls = photos.slice(0, 5)
      .map(p => resolvePhotoUrl(p, 800))
      .filter(u => u && u !== photoUrl);
    // Legacy: keep photo_reference for any web-service callers / tests.
    const altRefs = photos.slice(0, 4).map(p => p.photo_reference).filter(Boolean);
    return {
  ...
  function cacheKey(stop) {
    if (!stop) return null;
    // v2: shaped entries now carry resolved photoUrl/altPhotoUrls (JS SDK
    // getUrl) instead of unusable photo_reference strings. Bumping the version
    // invalidates the old photoless cache so the next load re-enriches.
    return `namibia_place_v2:${(stop.name || '').slice(0, 80)}:${Number(stop.lat).toFixed(2)}:${Number(stop.lng).toFixed(2)}`;
  }
