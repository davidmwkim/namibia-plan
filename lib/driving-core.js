...
(function (root, factory) {
  ...
  // Status is rule-derived from surface + zones, per Heather's own comfort:
  // GRAVEL and the heavy-traffic cities are RED (David); firm DIRT/SAND and the
  // benign Sesriem park tar are GREEN (Heather handles these — low traffic);
  // every other paved/town road is YELLOW (caution — her real concern is other
  // traffic: merging, passing, busy junctions). Curated prose layers on top.
  ...
  function rateStep(step, prevCtx) {
    ...
    // Driveability is about TRAFFIC, not surface (Heather's call): she handles
    // firm dirt + sand fine, but leaves loose GRAVEL to David. Her real concern
    // is other traffic — merging, passing, busy junctions — so paved roads are
    // caution and the heavy-traffic cities are David's.
    let status;
    if (roadSurface === 'gravel') status = 'no';                            // gravel → David (her call)
