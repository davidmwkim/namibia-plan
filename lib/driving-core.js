...
(function (root, factory) {
  ...
  // Smallest signed turn between two bearings, absolute (0..180).
  function angleDelta(a, b) { return Math.abs(((b - a + 540) % 360) - 180); }
  // How twisty a polyline is: total heading change per km (deg/km). ~0–25 for a
  // straight open road, 80+ for a hairpin pass. Used to tell "straight gravel
  // Heather is comfortable on" from "twisty pass that's David's".
  function pathCurviness(points) {
    if (!Array.isArray(points) || points.length < 3) return 0;
    let dist = 0, turn = 0;
    for (let i = 1; i < points.length; i++) dist += distMeters(points[i - 1], points[i]);
    for (let i = 1; i < points.length - 1; i++) turn += angleDelta(bearing(points[i - 1], points[i]), bearing(points[i], points[i + 1]));
    return dist > 0 ? turn / (dist / 1000) : 0;
  }
  ...
  function cardDistMeta(route) {
    ...
    const cardDist = cards.map(c => {
      ...
      // Scenery (5 km road-view) cards are a passive gallery — never the "active"
      // waypoint, so the active card stays a real turn/stop and doesn't jump
      // between the many scenery frames on a long step.
      if (c.kind === 'scenery') return Infinity;
  ...
  // ONE definition of how each road surface is drawn, so the map route
  // overlays AND the Overview/driving summary bar stay synchronized. `dash` is
  // a Leaflet dashArray; `color` tints the thin overlay drawn on top of the
  // Heather-coloured line (null = solid, i.e. paved needs no overlay). `barCss`
  // is the rich CSS class the summary bar uses.
  // `dash` = Leaflet dashArray for the parallel surface line; `mapColor` = that
  // line's colour (drawn OFFSET beside the Heather route line, not on top);
  // `barCss` = the rich CSS class the summary bar uses.
  const SURFACE_VIS = {
    paved:  { dash: null,       mapColor: '#1f2937', barCss: 'surf-paved'  },
    gravel: { dash: '8 6',      mapColor: '#6b7280', barCss: 'surf-gravel' },
    dirt:   { dash: '11 5 2 5', mapColor: '#92400e', barCss: 'surf-dirt'   },
    unpaved:{ dash: '11 5 2 5', mapColor: '#92400e', barCss: 'surf-dirt'   },
    sand:   { dash: '2 8',      mapColor: '#d4a017', barCss: 'surf-sand'   },
    urban:  { dash: '2 5',      mapColor: '#475569', barCss: 'surf-urban'  },
    mixed:  { dash: '8 6',      mapColor: '#6b7280', barCss: 'surf-mixed'  }
  };

  // ===================== Heather rating engine =====================
  // Status is rule-derived from surface + geometry + zones, per Heather's own
  // comfort: open, straight, low-traffic GRAVEL is GREEN (she's comfortable), as
  // are firm DIRT/SAND and the benign Sesriem park tar; TWISTY gravel (passes,
  // hairpins, junction clusters) and the heavy-traffic cities are RED (David);
  // every other paved/town road is YELLOW (caution — other traffic: merging,
  // passing, busy junctions). Twistiness comes from step.curvyDegPerKm
  // (pathCurviness), set during route enrichment. Curated prose layers on top.
  ...
  // Built-up areas: a sealed road whose midpoint sits here is TOWN driving (the
  // B1/approach roads through Windhoek are city streets, not open highway), so
  // the surface shows as 'urban' regardless of its road code.
  const URBAN_ZONES = [
    { lat: -22.570, lng: 17.083, rM: 9000, name: 'Windhoek' },
    { lat: -22.676, lng: 14.527, rM: 4500, name: 'Swakopmund' },
    { lat: -22.957, lng: 14.505, rM: 5000, name: 'Walvis Bay' }
  ];
  // Gravel twistiness ceiling (deg/km, from step.curvyDegPerKm = the step's
  // average heading-change per km): at or below this a gravel run is straight/
  // open enough overall that Heather is comfortable; above it the step is
  // predominantly twisty (a pass, hairpins, junction cluster) → David's. The
  // average (not the peak) is deliberate: a long, mostly-straight road with one
  // short pass still reads as "not a lot of turns" overall.
  const GRAVEL_CURVY_MAX = 80;
  ...
  function rateStep(step, prevCtx) {
    ...
    // roadSurface is the underlying road type (used for prevCtx inheritance so a
    // town override never leaks onto the next rural step).
    const roadSurface = info.surface || prevCtx.surface || 'paved';
    const code = info.code || prevCtx.code || null;
    // Classify by the step MIDPOINT (not its start) so a long step that merely
    // begins inside a town/green zone isn't mis-rated for its whole length
    // (e.g. the 85 km B1 that starts at Windhoek's edge).
    const p = { lat: Number(step && step.lat), lng: Number(step && step.lng) };
    const eLat = Number(step && step.endLat), eLng = Number(step && step.endLng);
    if (isFinite(eLat) && isFinite(eLng)) { p.lat = (p.lat + eLat) / 2; p.lng = (p.lng + eLng) / 2; }
    const loose = roadSurface === 'gravel' || roadSurface === 'dirt' || roadSurface === 'sand';
    // A sealed road inside a built-up area is town driving → 'urban' surface.
    const surface = (!loose && isFinite(p.lat) && inZone(p, URBAN_ZONES)) ? 'urban' : roadSurface;
    // Driveability is about TURNS + TRAFFIC, not the surface itself (Heather's
    // call): she's comfortable on open, straight, low-traffic GRAVEL (and on
    // firm dirt/sand), but leaves the twisty gravel (passes, hairpins, junction
    // clusters) and the heavy-traffic cities to David. Her concern on tar is
    // other traffic — merging, passing, busy junctions.
    const curvy = (step && typeof step.curvyDegPerKm === 'number') ? step.curvyDegPerKm : null;
    let status;
    if (roadSurface === 'gravel') {
      if (isFinite(p.lat) && inZone(p, RED_ZONES)) status = 'no';           // busy gravel near town
      else if (curvy != null && curvy > GRAVEL_CURVY_MAX) status = 'no';    // twisty — pass / many turns → David
      else status = 'yes';                                                  // open, straight, quiet → Heather's comfortable
    }
    else if (roadSurface === 'dirt' || roadSurface === 'sand') status = 'yes'; // firm dirt/sand → Heather, low traffic
    else if (isFinite(p.lat) && inZone(p, RED_ZONES)) status = 'no';        // busy city/harbour — heavy traffic + merging
    else if (isFinite(p.lat) && inZone(p, GREEN_ZONES)) status = 'yes';     // benign tar (Sesriem park road)
    else status = 'maybe';                                                  // paved/town — caution: traffic, merging, passing
    return { status, surface, code, roadSurface };
  }
  ...
  function heatherLegs(route) {
    ...
    route.legs.forEach(leg => (leg.steps || []).forEach(s => {
      const r = rateStep(s, prev); prev = { surface: r.roadSurface, code: r.code };
